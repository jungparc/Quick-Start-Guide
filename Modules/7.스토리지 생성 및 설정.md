...
1. `linux-server-basic` 에 원격 접속한 상태에서 아래 명령어를 실행하여 새롭게 생성한 블록 스토리지에 `mysql-db-basic` 데이터 조회 자료를 생성합니다.
...
2. 아래 명령어로 index.html 파일을 저장합니다.
...
11. 아래 명령어를 실행하여 추가 서비스 구성을 설정합니다.
    ...
    ```
12. 웹 브라우저에서 새 창을 열어서 `http://복사한 linux-server-basic 플로팅 IP 주소` 를 입력하여 변경된 웹 페이지를 확인합니다.
